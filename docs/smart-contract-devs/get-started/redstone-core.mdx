...
It is the most mature way to use RedStone, battle tested in production, protecting >$100M TVL of [Defi Protocols](https://defillama.com/oracles/RedStone) protocol where the price feeds were injected
to [~45K transactions](https://dune.com/hatskier/redstone).
...
## Manual payload
If your contracts are written in solidity in a version lower than `0.8.4` it could be problematic to extend from the `RedstoneConsumerBase` contract.
...
contract RedstoneExtractor is RedstoneConsumerNumericMock {
  ...
  function extractPrice(bytes32 feedId) public view returns(uint256) {
...
const price = await yourContract.getPriceFromRedstoneOracle(`0x${redstonePayload}`);
```
