---
sidebar_position: 1
sidebar_label: "⚙️ Pull (on-demand feeds)"
---
...
Pull model is the most mature way to use RedStone, battle tested in production, protecting >$100M TVL of [DeFi Protocols](https://defillama.com/oracles/RedStone) (not all listed yet) across multiple mainnets. The price feeds have been injected to more than [~215K transactions](https://dune.com/hatskier/redstone).
...
Even though the most popular way of using RedStone data is to pass it on-chain there one may want to consume it off-chain.
