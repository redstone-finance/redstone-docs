...
To analyze these interconnected systems, the methodology constructs "virtual markets"—analytical constructs that aggregate all positions linking one borrowed asset to each collateral asset. For instance, all positions borrowing USDC against ETH collateral constitute one virtual market, while USDC borrowed against wBTC forms another. Although these virtual markets may not exist on-chain, they are essential for risk decomposition. Each pair captures aggregate exposure, weighted loan-to-value (LTV) distributions, and concentration risk for that specific borrow-collateral relationship. This decomposition transforms complex unified pools or vault networks into analyzable components while preserving the systemic interactions between them.
